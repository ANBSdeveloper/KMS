IF OBJECT_ID(N'[RP_PosmInvestment_Produce]') IS NOT NULL
BEGIN
    DROP PROCEDURE [RP_PosmInvestment_Produce]
END

GO

CREATE PROC [dbo].[RP_PosmInvestment_Produce] -- [RP_PosmInvestment_Produce] 1, '2020-01-01', '2023-01-01', NULL
@UserId int,
@FromDate DateTime,
@ToDate DateTime,
@StaffId int = NULL
...
BEGIN
	...
	SELECT 
		...
		, StatusName = CASE 
							...
							WHEN itemInvest.Status = 20 THEN N'ASM từ chối duyệt đề xuất'
							...
							WHEN itemInvest.Status = 60 THEN N'RSM từ chối duyệt đề xuất'
							WHEN itemInvest.Status = 70 THEN N'RSM duyệt đề xuất'
							WHEN itemInvest.Status = 80 THEN N'Giám đốc từ chối duyệt đề xuất'
							WHEN itemInvest.Status = 90 THEN N'Giám đốc duyệt đề xuất'
							WHEN itemInvest.Status = 100 THEN N'Đơn hàng không hợp lệ'
							WHEN itemInvest.Status = 110 THEN N'SS đề xuất bổ sung chi phí'	
							WHEN itemInvest.Status = 120 THEN N'ASM xác nhận bổ sung chi phí'
							WHEN itemInvest.Status = 130 THEN N'RSM xác nhận bổ sung chi phí'
							WHEN itemInvest.Status = 140 THEN N'Trade xác nhận bổ sung chi phí'
							WHEN itemInvest.Status = 150 THEN N'Đơn hàng hợp lệ'
							WHEN itemInvest.Status = 160 THEN N'Xác nhận sản xuất 1'
							WHEN itemInvest.Status = 170 THEN N'Xác nhận sản xuất 2'
							WHEN itemInvest.Status = 180 THEN N'Xác nhận NCC sản xuất'
							WHEN itemInvest.Status = 190 THEN N'Nghiệm thu'
							WHEN itemInvest.Status = 200 THEN N'Nghiệm thu 1'
							WHEN itemInvest.Status = 210 THEN N'Nghiệm thu 2'
						END
			, OperationLink
			, OperationPhotoLink1 ='https://api-kms.vitadairy.vn/api/v1/posm-investments/items/' + CAST(itemInvest.Id AS VARCHAR) + '/operation-images/0'
			, OperationPhotoLink2 ='https://api-kms.vitadairy.vn/api/v1/posm-investments/items/' + CAST(itemInvest.Id AS VARCHAR) + '/operation-images/1'
			, OperationPhotoLink3 ='https://api-kms.vitadairy.vn/api/v1/posm-investments/items/' + CAST(itemInvest.Id AS VARCHAR) + '/operation-images/2'
			, OperationPhotoLink4 ='https://api-kms.vitadairy.vn/api/v1/posm-investments/items/' + CAST(itemInvest.Id AS VARCHAR) + '/operation-images/3'
	...
	LEFT JOIN Vendors AS vendor ON itemInvest.VendorId = vendor.Id
	WHERE  posmInvest.CreationTime >= @FromDate 
	AND posmInvest.CreationTime <= @ToDate       
	AND itemInvest.Status = 160
	AND  EXISTS(
			SELECT TOP 1 *
			FROM CTE 
