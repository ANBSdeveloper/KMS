...
{
    public class PosmInvestmentSalesRemarkCommandHandler : RequestHandlerBase, IRequestHandler<PosmInvestmentSalesRemarkCommand>
    {
        private readonly IRepository<PosmInvestment, int> _posmInvestmentRepository;
        private readonly DistributedLockManager _distributedLockManager;

        public PosmInvestmentSalesRemarkCommandHandler(DistributedLockManager distributedLockManager, IRequestSupplement supplement, IRepository<PosmInvestment, int> posmInvestmentRepository) : base(supplement)
        {
        ...
        public async Task<Unit> Handle(PosmInvestmentSalesRemarkCommand request, CancellationToken cancellationToken)
        {
            ...
            await using (await _distributedLockManager.AcquireAsync($"posm_investment_" + requestData.PosmInvestmentId))
            {
                ...
                await posmInvestment.ApplyActionAsync(new PosmInvestmentSalesRemarkAction(
                    IocResolver,
                    LocalizationSource,
