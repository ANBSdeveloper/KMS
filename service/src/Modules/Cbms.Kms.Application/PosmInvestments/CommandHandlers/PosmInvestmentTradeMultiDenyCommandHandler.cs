...
{
    public class PosmInvestmentTradeMultiDenyCommandHandler : RequestHandlerBase, IRequestHandler<PosmInvestmentTradeMultiDenyCommand>
    {
        private readonly IRepository<PosmInvestment, int> _posmInvestmentRepository;
        private readonly DistributedLockManager _distributedLockManager;

        public PosmInvestmentTradeMultiDenyCommandHandler(DistributedLockManager distributedLockManager, IRequestSupplement supplement, IRepository<PosmInvestment, int> posmInvestmentRepository) : base(supplement)
        {
        ...
        public async Task<Unit> Handle(PosmInvestmentTradeMultiDenyCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var id in request.Data.Ids)
            {
                ...
                .FirstOrDefaultAsync(p => p.Id == id);
                    ...
                    {
                        await posmInvestment.ApplyActionAsync(new PosmInvestmentTradeDenyRequestAction(
                            IocResolver,
                            LocalizationSource,
                            requestData.Note));
