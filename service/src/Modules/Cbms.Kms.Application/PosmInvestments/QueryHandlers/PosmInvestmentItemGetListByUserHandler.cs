...
{
    public class PosmInvestmentItemGetListByUserHandler : QueryHandlerBase, IRequestHandler<PosmInvestmentItemGetListByUser, PagingResult<PosmInvestmentItemExtDto>>
    {
        private readonly AppDbContext _dbContext;
        private readonly ISqlConnectionFactory _sqlConnectionFactory;

        public PosmInvestmentItemGetListByUserHandler(IRequestSupplement supplement, AppDbContext dbContext, ISqlConnectionFactory sqlConnectionFactory) : base(supplement)
        {
        ...
        public async Task<PagingResult<PosmInvestmentItemExtDto>> Handle(PosmInvestmentItemGetListByUser request, CancellationToken cancellationToken)
        {
            ...
            string pagingSql = $@"
                ...
                + (salesOrgId.HasValue ? $@"AND (c.SalesSupervisorStaffId = {staff.Id} OR c.AsmStaffId = {staff.Id} OR c.RsmStaffId = {staff.Id}) " : "")
                        + (request.Status.Count > 0 ? $@"AND (i.Status IN @Status OR iv.Status IN @Status) " : "")
            ...
            var items = await connection.QueryAsync<PosmInvestmentItemExtDto>(pagingSql, new { fromDate, toDate, Status = request.Status, request.WardId, request.DistrictId, request.ProvinceId });
