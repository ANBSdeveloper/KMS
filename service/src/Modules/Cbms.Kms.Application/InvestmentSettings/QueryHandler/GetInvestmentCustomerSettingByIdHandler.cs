...
{
    public class GetInvestmentCustomerSettingByIdHandler : QueryHandlerBase, IRequestHandler<GetInvestmentCustomerSettingById, InvestmentCustomerSettingDto>
    {
        ...
        public GetInvestmentCustomerSettingByIdHandler(
        ...
        public async Task<InvestmentCustomerSettingDto> Handle(GetInvestmentCustomerSettingById request, CancellationToken cancellationToken)
        {
            ...
            var customer = await _customerRepository.GetAll().FirstOrDefaultAsync(p => p.Id == request.CustomerId);
            if (customer == null)
            {
