import { ChangeDetectorRef, Component, HostBinding, Injector, Input, OnDestroy, OnInit } from '@angular/core';
...
import { AuthUser, User } from 'app/auth/models';
...
export class CoreMenuVerticalCollapsibleComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  checkItemHasChild(item: CoreMenuItem): boolean {
    if (item.children != undefined) {
      var hasChild = false;
      for (var i = 0; i < item.children.length; i++) {
        var childItem = item.children[i];
        if(this.currentUser.isGrantedAny(childItem.permissions)) {
          hasChild = this.checkItemHasChild(childItem);
          if (hasChild) {
            break;
          }
        }
