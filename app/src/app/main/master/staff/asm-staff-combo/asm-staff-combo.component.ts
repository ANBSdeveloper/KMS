...
//#endregion
@Component({
  selector: "app-asm-staff-combo",
  templateUrl: "./asm-staff-combo.component.html",
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: ASMStaffComboComponent,
    },
  ],
})
export class ASMStaffComboComponent extends ComboComponentBase {
  //#region Variables
  ...
  getListRequest(maxResult: number, skip: number, search: string) {
    if (this.supervisorId || this.getDataWithoutSupervisor) {
      return this.getDataService<DataServiceProxy>()
        .getAsmStaffs(this.supervisorId, maxResult, skip, search, "", undefined)
        .pipe(
          map((res) => ({
            result: {
